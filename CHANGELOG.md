...
- 目录分享默认排除所有 dotfile 和隐藏目录；如需包含隐藏文件，必须显式传入 `--include-hidden`，且分享前密钥扫描会同步覆盖这些隐藏文件。
...
- 新增隐藏文件排除 helper、密钥扫描单元与 CLI 集成测试，覆盖文件、目录、stdin、Git commit、`--force`、`--no-secret-scan`、隐藏文件默认排除、`--include-hidden`、显式排除优先级和脱敏输出。
- 新增 Commits UI 回归测试，防止 diff 新增/删除行高亮和 Recent commits 高对比度文本样式回退。
